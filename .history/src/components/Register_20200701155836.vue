<template>
  <el-dialog title="注册" width="560px" :visible.sync="dialogVisible" @close="handleClose">
    <el-form :model="form" ref="form" label-width="80px" class="dialog-form">
      ...
      <el-form-item label="确认密码" :rules="[{ required: true, message: '请确认密码', trigger: 'blur' }, { validator: passwordValidator, trigger: 'blur' }]">
        <el-input maxlength="20" type="password" v-model="form.comPassword" placeholder="请确认密码"></el-input>
      </el-form-item>
      <el-form-item label="邮箱" :rules="[{ required: true, message: '请输入邮箱', trigger: 'blur' }, { validator: emailValidator, trigger: 'blur' }]">
        <el-input maxlength="100" v-model="form.email" placeholder="请输入邮箱"></el-input>
      </el-form-item>
      <el-form-item label="手机号码" :rules="[{ validator: phoneValidator, trigger: 'blur' }]">
        <el-input maxlength="11" row="4" v-model="form.phone" placeholder="请输入您的手机号码"></el-input>
      </el-form-item>
      <el-form-item label="个人介绍" :rules="[{ required: true, message: '请输入账户名称', trigger: 'blur' },  { validator: emailValidator, trigger: 'change'},]">
